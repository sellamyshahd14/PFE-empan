...
class PatientAnalyticsDashboard extends StatefulWidget {
  ...
  @override
  State<PatientAnalyticsDashboard> createState() =>
      _PatientAnalyticsDashboardState();
}
              ...
              builder: (context, constraints) {
                if (constraints.maxWidth > 800) {
                  return Row(
                    ...
                    children: [
                      Expanded(
                        child: DsmComparisonChart(
                          state: _state,
                          selectedDate: _selectedSession,
                        ),
                      ),
                      const SizedBox(width: 20),
...
class ClinicalAlertBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<String> alerts = [];
    if (metrics['dsmMean'] != null && metrics['dsmMean'] < 40)
      alerts.add("Score DSM-48 inférieur au seuil critique (< 40/48)");
    if (metrics['hadsA'] != null && metrics['hadsA'] >= 11)
      alerts.add("Symptomatologie anxieuse significative (HADS-A ≥ 11)");
    if (metrics['hadsD'] != null && metrics['hadsD'] >= 11)
      alerts.add("Symptomatologie dépressive significative (HADS-D ≥ 11)");
    if (metrics['tmtBAvg'] != null && metrics['tmtBAvg'] > 180)
      alerts.add("Déficit exécutif sévère (TMT B > 180s)");
    ...
    if (metrics['empanMaFlag'] == true)
      alerts.add("Ratio Empan Inverse/Direct < 0.7 (Signe MA)");
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              "Cette session justifie une réévaluation approfondie.",
              style: GoogleFonts.cairo(
                fontSize: 14,
                fontWeight: FontWeight.bold,
              ),
...
class SessionSelectorBar extends StatelessWidget {
  ...
  bool _hasAlert(String session) {
    ...
    double empanMax = max(
      (m['empanDirect'] ?? 0).toDouble(),
      (m['empanInverse'] ?? 0).toDouble(),
    );
  ...
  Widget build(BuildContext context) {
    String completeness =
        state.completenessLabels[selectedSession] ?? 'Inconnue';
    Color compColor = completeness == 'Complète'
        ? Colors.green
        : (completeness == 'Partielle' ? Colors.orange : Colors.red);
        ...
        children: [
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: state.sortedSessions.map((session) {
                  bool complete =
                      state.completenessLabels[session] == 'Complète';
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: alert
                                ? Colors.orange
                                : (complete ? Colors.green : Colors.grey),
                          ),
                        ),
                        const SizedBox(width: 8),
                        Text(session),
          ...
          Row(
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () =>
                    PdfExportService.exportPatientReport(patientId, state),
                icon: const Icon(
...
class CognitiveDomainCards extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color dsmColor = dsm == null
        ? Colors.grey
        : (dsm >= 40 ? Colors.green : Colors.red);
    String dsmVal = dsm == null
        ? "Non évalué"
        : "${dsm.toStringAsFixed(1)} / 48";
    ...
    String eVal = "";
    if (eDir != null && eInv != null)
      eVal = "D: ${eDir.toInt()} | I: ${eInv.toInt()}";
    else if (eDir != null)
      eVal = "D: ${eDir.toInt()}";
    else if (eInv != null)
      eVal = "I: ${eInv.toInt()}";
    ...
    Color doColor = do30 == null
        ? Colors.grey
        : (do30 >= 27
              ? Colors.green
              : (do30 >= 20 ? Colors.orange : Colors.red));
    ...
    if (tmtA != null && tmtB != null) {
      ...
      tmtColor = diff < 60
          ? Colors.green
          : (diff <= 120 ? Colors.orange : Colors.red);
    } else if (tmtA != null) {
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _buildCard(
                "Attention (Empan)",
                "Attention — État : $eStatus",
                eVal,
                Icons.graphic_eq,
                Colors.blue,
                empanColor,
                alertBadge: eAlert ? "Alerte CLINIQUE ⚠" : null,
              ),
            ),
          ],
        ),
              ...
              Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: iconBase.withOpacity(0.1),
                      shape: BoxShape.circle,
                    ),
...
class HadsDualGauge extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Text(
              "Score > 10 sur l'une ou l'autre sous-échelle requiert une attention clinique",
              style: GoogleFonts.cairo(
                fontSize: 10,
  ...
  Widget _buildSfGauge(
    ...
    if (prevScore != null && prevDate != null) {
      ...
      if (diff < 0) {
        trendStr = "▼ ${diff.abs().toInt()} pts depuis $prevDate";
        trendCol = Colors.green;
      } else if (diff > 0) {
        trendStr = "▲ ${diff.toInt()} pts depuis $prevDate";
        trendCol = Colors.red;
      } else {
        trendStr = "− Stable depuis $prevDate";
        trendCol = Colors.grey;
      }
    }
      ...
      children: [
        ...
        if (trendStr.isNotEmpty)
          Padding(
            ...
            child: Text(
              trendStr,
              style: GoogleFonts.cairo(fontSize: 10, color: trendCol),
            ),
          ),
      ],
    );
...
class LongitudinalTrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (String s in chronological) {
      ...
      if (m != null) {
        if (m['dsmMean'] != null)
          dsmSpots.add(FlSpot(i.toDouble(), (m['dsmMean'] / 48.0) * 100));
        if (m['do30'] != null)
          doSpots.add(FlSpot(i.toDouble(), (m['do30'] / 30.0) * 100));
        if (m['empanDirect'] != null)
          empanSpots.add(FlSpot(i.toDouble(), (m['empanDirect'] / 9.0) * 100));
      }
      final g = state.globalIndexHistory.firstWhere(
        (element) => element['date'] == s,
        orElse: () => {},
      );
      if (g.isNotEmpty && g['index'] != null)
        globalSpots.add(FlSpot(i.toDouble(), g['index']));
      i++;
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            "Évolution Longitudinale des Profils",
            style: GoogleFonts.cairo(
              fontSize: 16,
              fontWeight: FontWeight.bold,
              color: Colors.blueGrey[800],
                  ...
                  touchCallback:
                      (FlTouchEvent event, LineTouchResponse? response) {
                        if (event is! FlTapUpEvent)
                          return; // Prevents switching session while scrolling
          ...
          Row(
            ...
            children: [
              _legendItem(Colors.teal, "DSM"),
              const SizedBox(width: 16),
              _legendItem(Colors.purple, "DO-30"),
              const SizedBox(width: 16),
              _legendItem(Colors.blue, "Empan Dir."),
              const SizedBox(width: 16),
...
class TmtFlexibilityChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (String session in chronological) {
      final metrics = state.sessionAverages[session];
      if (metrics != null &&
          metrics['tmtAAvg'] != null &&
          metrics['tmtBAvg'] != null) {
                        ...
                        touchTooltipData: LineTouchTooltipData(
                          getTooltipItems: (touchedSpots) {
                            return touchedSpots.map((spot) {
                              ...
                              String interp = spot.y <= 60
                                  ? "Normal"
                                  : (spot.y <= 120 ? "Modéré" : "Sévère");
                              return LineTooltipItem(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(
                dsmMean != null
                    ? "Moy.: ${dsmMean.toStringAsFixed(1)}/48"
                    : "Non évalué cette session",
                style: GoogleFonts.cairo(
                  fontWeight: FontWeight.bold,
                  color: dsmMean != null ? Colors.teal[700] : Colors.grey,
                ),
              ),
            ],
          ),
                ...
                : BarChart(
                    BarChartData(
                      ...
                      extraLinesData: ExtraLinesData(
                        horizontalLines: [
                          HorizontalLine(
                            y: 40.0, // CLINICAL THRESHOLD UPDATED
                            color: Colors.redAccent.withOpacity(0.7),
                            strokeWidth: 2,
                            dashArray: [5, 5],
                            label: HorizontalLineLabel(
                              show: true,
                              labelResolver: (line) => "Seuil clinique (40/48)",
                              style: GoogleFonts.cairo(
