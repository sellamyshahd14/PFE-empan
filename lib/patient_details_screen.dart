...
class PatientDetailsScreen extends StatelessWidget {
                ...
                children: [
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(date, style: GoogleFonts.cairo(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
                      ],
                    ),
                  ),
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: [
                        DataColumn(label: Text("Test Name", style: GoogleFonts.cairo(fontWeight: FontWeight.bold))),
                        DataColumn(label: Text("Criteria 1", style: GoogleFonts.cairo(fontWeight: FontWeight.bold))),
                        DataColumn(label: Text("Criteria 2", style: GoogleFonts.cairo(fontWeight: FontWeight.bold))),
                        DataColumn(label: Text("Actions", style: GoogleFonts.cairo(fontWeight: FontWeight.bold))),
                      ],
                      rows: dayResults.map((res) {
                        ...
                        final metadataMap = (data['metadata'] as Map<String, dynamic>?) ?? data;

                        Widget criteria1Widget;
                        Widget criteria2Widget;

                        if (testType == "TMT-A" || testType == "TMT-B" || testType == "TMT-C") {
                          final totalActions = metadataMap['totalActions'] ?? (metadataMap['path'] as List?)?.length ?? 0;
                          final mistakes = metadataMap['mistakes'] ?? metadataMap['errors'] ?? 0;
                          final required = testType == "TMT-A" ? 25 : (testType == "TMT-B" ? 50 : 16);

                          criteria1Widget = Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text("Actions: $totalActions/$required", style: GoogleFonts.cairo(fontSize: 13, color: Colors.blue.shade900, fontWeight: FontWeight.bold)),
                              Text("Erreurs: $mistakes/$totalActions", style: GoogleFonts.cairo(fontSize: 13, color: Colors.orange.shade900)),
                            ],
                          );
                          criteria2Widget = Container(
                            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                            decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8)),
                            child: Text("Temps: ${_formatDuration(duration)}", style: GoogleFonts.cairo(fontWeight: FontWeight.w600, color: Colors.black87)),
                          );
                           ...
                           criteria1Widget = Column(
                             ...
                             crossAxisAlignment: CrossAxisAlignment.start,
                             children: [
                               Text("Anxiété: $scoreA", style: GoogleFonts.cairo(fontWeight: FontWeight.w600)),
                               Text(_getHadsLevel(scoreA), style: GoogleFonts.cairo(fontSize: 13, color: Colors.orange.shade900, fontWeight: FontWeight.bold)),
                             ],
                           );
                           criteria2Widget = Column(
                             ...
                             crossAxisAlignment: CrossAxisAlignment.start,
                             children: [
                               Text("Dépression: $scoreD", style: GoogleFonts.cairo(fontWeight: FontWeight.w600)),
                               Text(_getHadsLevel(scoreD), style: GoogleFonts.cairo(fontSize: 13, color: Colors.orange.shade900, fontWeight: FontWeight.bold)),
                             ],
                           );
                        } else {
                          criteria1Widget = Container(
                            ...
                            child: Text("Score: $score pts", style: GoogleFonts.cairo(fontWeight: FontWeight.w600, color: Colors.black87)),
                          );
                          criteria2Widget = Container(
                            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                            decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8)),
                        ...
                        return DataRow(
                          cells: [
                            DataCell(Text(testType, style: GoogleFonts.cairo(fontWeight: FontWeight.bold, color: Colors.teal))),
                            ...
                            DataCell(
                              Row(
                                ...
                                children: [
                                  if (testType.startsWith("TMT") && metadataMap['path'] != null)
                                    IconButton(
                                      icon: const Icon(Icons.visibility, color: Colors.teal),
                                      onPressed: () => _showTmtVisualizationDialog(context, testType, metadataMap),
                                    ),
                                  if (testType.startsWith("DSM-48") || testType == "DO-30" || testType.toLowerCase().contains('empan'))
                                    IconButton(
                                      icon: const Icon(Icons.list_alt, color: Colors.blue),
                                      onPressed: () => _showTestDetailsDialog(context, testType, metadataMap),
                                    ),
                                  if (testType.contains("HADS") && metadataMap['hadsFormat'] != null)
                                    IconButton(
                                      icon: const Icon(Icons.description_outlined, color: Colors.teal),
                                      onPressed: () => _showHadsFormDialog(context, metadataMap),
  ...
  void _showTestDetailsDialog(BuildContext context, String testType, Map<String, dynamic> metadata) {
            ...
            : SingleChildScrollView(
                child: DataTable(
                  ...
                  columns: [
                    ...
                    if (isDsm48) const DataColumn(label: Text("Pat")),
                    const DataColumn(label: Text("")),
                  ],
                  rows: tableData.map((item) {
                    final bool isCorrect = item['isCorrect'] ?? item['correct'] ?? false;
                    
                    if (isDsm48) {
                      return DataRow(
                        cells: [
                          DataCell(Text("${item['numero']}")),
                          DataCell(Text(item['categorie'] ?? "", style: GoogleFonts.cairo(fontSize: 12))),
                      ...
                      final String expected = item['reponseAttendueAr'] ?? item['reponseAttendueFr'] ?? '';
                      return DataRow(
                        cells: [
                          DataCell(Text("${item['numero']}")),
  ...
  void _showHadsFormDialog(BuildContext context, Map<String, dynamic> metadata) {
                ...
                child: Column(
                  children: entries.map((item) {
                    ...
                    return Card(
                      ...
                      color: type == 'A' ? Colors.blue.shade50 : Colors.orange.shade50,
                      child: ListTile(
                        leading: CircleAvatar(
                          backgroundColor: type == 'A' ? Colors.blue.shade700 : Colors.orange.shade700,
                          radius: 12,
                          child: Text(type, style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                        ),
                        title: Text(questionText, style: GoogleFonts.cairo(fontSize: 13, fontWeight: FontWeight.bold)),
                        subtitle: Text(answerText, style: GoogleFonts.cairo(fontSize: 12, color: Colors.black87)),
                        trailing: CircleAvatar(
                          backgroundColor: Colors.white,
                          radius: 15,
                          child: Text("$score", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                        ),
                      ),
                    );
                  }).toList(),
  ...
  void _showTmtVisualizationDialog(BuildContext context, String testType, Map<String, dynamic> metadata) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          height: 600,
          child: path.isEmpty 
            ? const Center(child: Text("Aucun tracé enregistré pour ce test."))
            : Column(
                children: [
                   Expanded(
